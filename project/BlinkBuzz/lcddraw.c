...
void drawChar8x12(u_char col, u_char row, char c, u_int fgColorBGR, u_int bgColorBGR){
  ...
  lcd_setArea(col, row, col + 7, row + 12); // related to the requewsted
  while (rrow < 13) {
    while (rcol < 8) {
      u_int colorBGR = (font_8x12[oc][rcol] & bit) ? fgColorBGR : bgColorBGR;
      lcd_writeColor(colorBGR);
      rcol++;
    }
    rcol = 0;
...
void drawTriangle(int size, u_int COLOR, int off) {

  int x = 0; // initation variables 
  int y = 0;

  //draw filled in triangle
  for (x = 0; x <=  size; x++) { //rows
    for(y = 0; y <= size + x; y++){ //colums
      drawPixel(off - y, x, COLOR);
      drawPixel(off + y, x, COLOR);
    }
  }
}

//draws a custom random shape 
void drawCustom (int size, int off, u_int COLOR){
  //initiate what we will need
  int x;
  int y;
  int d;
